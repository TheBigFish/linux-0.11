...
#define __LIBRARY__  // 定义该变量是为了包括定义在unistd.h
                     // 中的内嵌汇编代码等信息。
 ...
 * 直到一个执行execve
 * 调用。这对堆栈可能带来问题。处理的方法是在fork()调用之后不让main()使用
 * 任何堆栈。因此就不能有函数调用 - 这意味着fork
 * 也要使用内嵌的代码，否则我们在从fork()退出 时就要使用堆栈了。 实际上只有pause
 * 和fork 需要使用内嵌方式，以保证从main()中不会弄乱堆栈，但是我们同时还
 * 定义了其它一些函数。
 */
static inline _syscall0(
    int, fork)  // 是unistd.h 中的内嵌宏代码。以嵌入汇编的形式调用
                // Linux 的系统调用中断0x80。该中断是所有系统调用的
                // 入口。该条语句实际上是int fork()创建进程系统调用。
                // syscall0 名称中最后的0 表示无参数，1 表示1 个参数。
    static inline _syscall0(int,
                            pause)  // int pause()系统调用：暂停进程的执行，直到
                                    // 收到一个信号。
    static inline _syscall1(int, setup, void *,
                            BIOS)  // int setup(void * BIOS)系统调用，仅用于
                                   // linux 初始化（仅在这个程序中被调用）。
    static inline _syscall0(
        int,
        sync)             // int sync()系统调用：更新文件系统。
...
extern int
vsprintf();  // 送格式化输出到一字符串中（在kernel/vsprintf.c，92 行）。
extern void init(void);  // 函数原形，初始化（在168 行）。
extern void blk_dev_init(
    void);  // 块设备初始化子程序（kernel/blk_drv/ll_rw_blk.c,157 行）
extern void chr_dev_init(
    void);  // 字符设备初始化（kernel/chr_drv/tty_io.c, 347 行）
extern void hd_init(void);  // 硬盘初始化程序（kernel/blk_drv/hd.c, 343 行）
extern void floppy_init(
    void);  // 软驱初始化程序（kernel/blk_drv/floppy.c, 457 行）
extern void mem_init(long start,
                     long end);  // 内存管理初始化（mm/memory.c, 399 行）
extern long rd_init(long mem_start,
                    int length);  //虚拟盘初始化(kernel/blk_drv/ramdisk.c,52)
 ...
 * 关于实时时钟更多的资料，那我很感兴趣。这些都是试探出来的，以及看了一些bios
 * 程序，呵！
 */
...
#define BCD_TO_BIN(val) \
  ((val) = ((val)&15) + ((val) >> 4) * 10)  // 将BCD 码转换成数字。
...
{               /* The startup routine assumes (well, ...) this */
  ...
  if (memory_end > 12 * 1024 * 1024)  // 如果内存>12Mb，则设置缓冲区末端=4Mb
    buffer_memory_end = 4 * 1024 * 1024;
           ...
           6 * 1024 * 1024)  // 否则如果内存>6Mb，则设置缓冲区末端=2Mb
  ...
  sched_init();  // 调度程序初始化(加载了任务0 的tr, ldtr)
                 // （kernel/sched.c，385）
  buffer_init(
      buffer_memory_end);  // 缓冲管理初始化，建内存链表等。（fs/buffer.c，348）
...
// 采用的格式，参见各种标准C 语言书籍。该子程序正好是vsprintf
// 如何使用的一个例子。 该程序使用vsprintf()将格式化的字符串放入printbuf
// 缓冲区，然后用write()将缓冲区的内容 输出到标准设备（1--stdout）。
{
  va_list args;
  int i;
...
void init(void) {
  ...
  // 该函数是在25
  // 行上的宏定义的，对应函数是sys_setup()，在kernel/blk_drv/hd.c,71 行。
  ...
  printf("Free mem: %d bytes\n\r",
         memory_end - main_memory_start);  //空闲内存字节数。
  // 下面fork()用于创建一个子进程(子任务)。对于被创建的子进程，fork()将返回0
  // 值， 对于原(父进程)将返回子进程的进程号。所以180-184
  // 句是子进程执行的内容。该子进程 关闭了句柄0(stdin)，以只读方式打开/etc/rc
  // 文件，并执行/bin/sh 程序，所带参数和 环境变量分别由argv_rc 和envp_rc
  // 数组给出。参见后面的描述。
  if (!(pid = fork())) {
    close(0);
  ...
  // 这三句的作用是父进程等待子进程的结束。&i
  // 是存放返回状态信息的位置。如果wait()返回值不 等于子进程号，则继续等待。
  if (pid > 0)
    while (pid != wait(&i)) /* nothing */;
  // 如果执行到这里，说明刚创建的子进程的执行已停止或终止了。下面循环中首先再创建一个子进程，
  // 如果出错，则显示“初始化程序创建子进程失败”的信息并继续执行。对于所创建的子进程关闭所有
  // 以前还遗留的句柄(stdin, stdout,
  // stderr)，新创建一个会话并设置进程组号，然后重新打开 /dev/tty0
  // 作为stdin，并复制成stdout
  // 和stderr。再次执行系统解释程序/bin/sh。但这次执行所
  // 选用的参数和环境数组另选了一套（见上面165-167
  // 行）。然后父进程再次运行wait()等待。如果
  // 子进程又停止了执行，则在标准输出上显示出错信息“子进程pid
  // 停止了运行，返回码是i”，然后 继续重试下去…，形成“大”死循环。
  while (1) {
    if ((pid = fork()) < 0) {
