...
track: 
    ...
    ! 从软盘加载数据到内存
    ! 注意： 
    ! 1、软盘有两个磁头，两个面，每面都有磁道
    ! 2、读完磁道第一个面所有扇区再读该磁道第二个面扇区
    ! 3、根据每磁道扇区数判断该面是否读完
read_it:
    ! 测试输入的段值。必须位于内存地址64KB 边界处，否则进入死循环。
...
ok2_read:
    ...
    ! 扇区编号变化按照 磁道（0-79）磁头（0-1）扇区（1-18） 从低到高递增
    ! 所以 0面扇区读完后接着读1面扇区，再递增磁道
    jne ok4_read    ! 如果是0 磁头，则再去读1 磁头面上的扇区数据。
    inc track       ! 否则去读下一磁道。
ok4_read:
    mov head,ax     ! 保存当前磁头号。（磁头0变为1）
