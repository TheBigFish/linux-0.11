...
volatile void do_exit(
    long code);  // 进程退出处理函数，在kernel/exit.c，102 行。
...
// 刷新页变换高速缓冲宏函数。
// 为了提高地址转换的效率，CPU
// 将最近使用的页表数据存放在芯片中高速缓冲中。在修改过页表
// 信息之后，就需要刷新该缓冲区。这里使用重新加载页目录基址寄存器cr3
...
// linux 0.11
// 内核默认支持的最大内存容量是16M，可以修改这些定义以适合更多的内存。
...
// CODE_SPACE(addr) ((((addr)+0xfff)&~0xfff) < current->start_code +
// current->end_code)。 该宏用于判断给定地址是否位于当前进程的代码段中，参见252
// 行。
...
// 内存映射字节图(1 字节代表1
// 页内存)，每个页面对应的字节用于标志页面当前被引用（占用）次数。
...
// 页表从物理地址0x1000 处开始（紧接着目录空间），每个页表有1024 项，也占4K
// 内存。 每个页表项对应一页物理内存（4K）。目录项和页表项的大小均为4 个字节。
...
int free_page_tables(unsigned long from, unsigned long size) {
  ...
  // 下面一句计算起始目录项。对应的目录项号=from>>22，因每项占4
  // 字节，并且由于页目录是从 物理地址0
  // 开始，因此实际的目录项指针=目录项号<<2，也即(from>>20)。与上0xffc 确保
  // 目录项指针范围有效。
...
int copy_page_tables(unsigned long from, unsigned long to, long size) {
  ...
  for (; size-- > 0; from_dir++, to_dir++) {
    ...
    // 为目的页表取一页空闲内存，如果返回是0
    // 则说明没有申请到空闲内存页面。返回值=-1，退出。
    if (!(to_page_table = (unsigned long *)get_free_page()))
      return -1; /* Out of memory, see freeing */
    // 设置目的目录项信息。7 是标志信息，表示(Usr, R/W, Present)。
    *to_dir = ((unsigned long)to_page_table) | 7;
    // 针对当前处理的页表，设置需复制的页面数。如果是在内核空间，则仅需复制头160
    // 页，否则需要 复制1 个页表中的所有1024 页面。
    nr = (from == 0) ? 0xA0 : 1024;
    ...
    for (; nr-- > 0; from_page_table++, to_page_table++) {
      ...
      if (!(1 & this_page))  // 如果当前源页面没有使用，则不用复制。
        continue;
      // 复位页表项中R/W 标志(置0)。(如果U/S 位是0，则R/W 就没有作用。如果U/S
      // 是1，而R/W 是0， 那么运行在用户层的代码就只能读页面。如果U/S 和R/W
      // 都置位，则就有写的权限。)
      this_page &= ~2;
      *to_page_table = this_page;  // 将该页表项复制到目的页表中。
      // 如果该页表项所指页面的地址在1M
      // 以上，则需要设置内存页面映射数组mem_map[]，于是计算
...
unsigned long put_page(unsigned long page, unsigned long address) {
  ...
  if ((*page_table) & 1)
    ...
    // 否则，申请空闲页面给页表使用，并在对应目录项中置相应标志7（User, U/S,
    // R/W）。然后将 该页表的地址??page_table。
    if (!(tmp = get_free_page())) return 0;
    *page_table = tmp | 7;
    page_table = (unsigned long *)tmp;
  }
  // 在页表中设置指定地址的物理内存页面的页表项内容。每个页表共可有1024
  // 项(0x3ff)。
...
void un_wp_page(unsigned long *table_entry) {
  ...
  // 被引用1 次，页面没有被共享），则在该页面的页表项中置R/W
  // 标志（可写），并刷新页变换 高速缓冲，然后返回。
  ...
  // 否则，在主内存区内申请一页空闲页面。
  if (!(new_page = get_free_page())) oom();  // Out of Memory。内存不够处理。
  // 如果原页面大于内存低端（则意味着mem_map[]>1，页面是共享的），则将原页面的页面映射
  // 数组值递减1。然后将指定页表项内容更新为新页面的地址，并置可读写等标志(U/S,
  // R/W, P)。 刷新页变换高速缓冲。最后将原页面内容复制到新页面。
...
void get_empty_page(unsigned long address) {
  ...
  // 279 行上英文注释的含义是：即使执行get_free_page()返回0
  // 也无所谓，因为put_page() 中还会对此情况再次申请空闲物理页面的，见210 行。
...
static int try_to_share(unsigned long address, struct task_struct *p) {
  ...
  if ((phys_addr & 0x41) != 0x01) return 0;
  // 取页面的地址??phys_addr。如果该页面地址不存在或小于内存低端(1M)也返回退出。
  phys_addr &= 0xfffff000;
  if (phys_addr >= HIGH_MEMORY || phys_addr < LOW_MEM) return 0;
  // *** 对当前进程页面进行操作。
  // 取页目录项内容??to。如果该目录项无效(P=0)，则取空闲页面，并更新to_page
  // 所指的目录项。
...
static int share_page(unsigned long address) {
  ...
  for (p = &LAST_TASK; p > &FIRST_TASK; --p) {
    ...
    if (current == *p)  // 如果就是当前任务，也继续寻找。
      continue;
    if ((*p)->executable !=
        current->executable)  // 如果executable 不等，也继续。
      continue;
...
void do_no_page(unsigned long error_code, unsigned long address) {
  ...
  // 若当前进程的executable
  // 空，或者指定地址超出代码+数据长度，则申请一页物理内存，并映射
  // 影射到指定的线性地址处。executable 是进程的i
  // 节点结构。该值为0，表明进程刚开始设置，
  // 需要内存；而指定的线性地址超出代码加数据长度，表明进程在申请新的内存空间，也需要给予。
  // 因此就直接调用get_empty_page()函数，申请一页物理内存并映射到指定线性地址处即可。
  // start_code 是进程代码段地址，end_data 是代码加数据长度。对于linux
  // 内核，它的代码段和 数据段是起始基址是相同的。
  ...
  /* 记住，（程序）头要使用1 个数据块 */
  // 首先计算缺页所在的数据块项。BLOCK_SIZE = 1024 字节，因此一页内存需要4
  // 个数据块。
  ...
  // 在增加了一页内存后，该页内存的部分可能会超过进程的end_data
  // 位置。下面的循环即是对物理 页面超出的部分进行清零处理。
...
//// 物理内存初始化。
// 参数：start_mem - 可用作分页处理的物理内存起始位置（已去除RAMDISK
// 所占内存空间等）。 end_mem - 实际物理内存最大地址。 在该版的linux
// 内核中，最多能使用16Mb 的内存，大于16Mb 的内存将不于考虑，弃置不用。 0 - 1Mb
// 内存空间用于内核系统（其实是0-640Kb）。
